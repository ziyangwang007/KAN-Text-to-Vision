...
from metrics import get_poses_ranks, __compare_pred_to_video
...
def test_distance(model, model_name, dataset, keypoints_path, num_samples=20):
    ...
    with torch.no_grad():
        ...
        for datum in dataset:
            ...
            pred2label_distance = __compare_pred_to_video(predicted_pose, keypoints_path, datum["id"], distance_function=fastdtw)
            pred2label_distances[datum["id"]] = pred2label_distance
            print(f"{datum['id']} distance: {pred2label_distance}")
...
if __name__ == "__main__":
    ...
    test(model, args["model_name"], dataset, test_seq_len_predictor=True, test_ranks=True,
