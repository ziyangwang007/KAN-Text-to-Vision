...
with open(f"{rootdir}/data/pjm_left_videos.json", 'r') as f:
...
def get_tfds_dataset(name, poses="openpose", fps=25, split="train", components: List[str] = None,
    ...
    # print(pose_header.components)
    # for component in pose_header.components:
...
def process_datum(datum, pose_header: PoseHeader, components: List[str] = None, normalize: bool = True) -> \
    ...
    for key, tf_pose in tf_poses.items():
        ...
        # todo 构建pose对象
        if datum["id"] in PJM_LEFT_VIDEOS_LST:  # 如果视频是左手视频
            pose = flip_pose(pose)  # 翻转
    ...
    # print(poses[""].header.components[1].name)
    # print(poses[""].header.components[1].points)
    # print(poses[""].header.components[1].limbs)
    # print(poses[""].header.dimensions)
    return {
