...
# utils
def get_pose_metric(keypoints_path: str, datum_id: str, fps: int = 25):
...
def compare_pose_videos(pose1_id, pose2_id, keypoints_path, distance_function=fastdtw):
    pose1 = get_pose_metric(os.path.join(keypoints_path, pose1_id), pose1_id)
    pose2 = get_pose_metric(os.path.join(keypoints_path, pose2_id), pose2_id)
...
def compare_pose_dtw(pose1, pose2, distance_function=fastdtw):
    poses_data = get_pose_data([pose1, pose2])
    total_distance = 0
    idx2weight = get_idx2weight(pose1.body.data.shape[2])
    for keypoint_idx, weight in idx2weight.items():
...
def __compare_pred_to_video(pred, keypoints_path, pose_id, distance_function=fastdtw):
    label_pose = get_pose_metric(os.path.join(keypoints_path, pose_id), pose_id)
    return compare_poses(pred, label_pose, distance_function=distance_function)

def __compare_pred_to_video_dtw(pred, keypoints_path, pose_id, distance_function=fastdtw):
    label_pose = get_pose_metric(os.path.join(keypoints_path, pose_id), pose_id)
    return compare_pose_dtw(pred, label_pose, distance_function=distance_function)
