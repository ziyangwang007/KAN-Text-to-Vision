...
def calculate_dtw_mje(pred_pose, gt_pose):
    """
    Calculate DTW-MJE for a single pair of predicted and ground truth poses.
    """
    num_joints = pred_pose.shape[1]
    total_mje = 0

    for joint in range(num_joints):
        pred_joint_seq = pred_pose[:, joint, :]
        gt_joint_seq = gt_pose[:, joint, :]

        distance, path = fastdtw(pred_joint_seq, gt_joint_seq, dist=euclidean)

        joint_error = sum(euclidean(pred_joint_seq[i], gt_joint_seq[j]) for i, j in path) / len(path)
        total_mje += joint_error

    return total_mje / num_joints
